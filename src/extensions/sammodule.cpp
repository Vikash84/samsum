...
static PyObject *get_mapped_reads(PyObject *self, PyObject *args) {
    ...
    */
    ...
    if ( verbose )
        cout << "Identifying reads with multiple alignments..." << std::flush;
    ...
    if ( verbose )
        cout << "Assigning weights to reads based on number of alignments..." << std::flush;
    assign_read_weights(mapped_reads, reads_dict);
    if ( verbose )
        cout << "done." << endl;

    if ( verbose )
        cout << "Removing reads with low quality alignments..." << std::flush;
